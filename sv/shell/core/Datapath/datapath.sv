...
logic dummycout;
logic [2:0] pc_add;
assign pc_add =  3'b100; //compressed ? 3'b010 :
assign PC_Minus_2 = PC_initial_OUT -2;
assign PC_OUT = compressed ? (PC_Minus_2) : PC_initial_OUT;
carry_lookahead_adder pc_adder (.a(PC_OUT[31:0]),.b({29'b0,pc_add}),.Subtract(0),.cin(0),.s(PC_NEXT),.cout(dummycout)); 
...
reg_file REG_FILE (.clk(clk),.WE(RegWriteW),.WD3(ResultW),.A1(rs1D),.A2(rs2D),.A3(RdW),.RD1(RD1D),.RD2(RD2D),.specialReg(specialReg)); //A1 & 2 are for reading, A3 is for writing
...
DE_REG DE_REG(.clk(clk),.RD1(RD1D),.RD2(RD2D),.PC_IN(PCD),.PC_PLUS(PCPlus4D),.ImmExt(ImmExtD),.RegWrite(RegWriteD),
                ...
                .instrD(instrD),.instrE(instrE),.lr(lr),.lrE(lrE),.sc(sc),.scE(scE));

logic [31:0] RD1E, RD2E, PCE, PCPlus4E, ImmExtE;
logic RegWriteE, MemWriteE, JumpE, BranchE, ALUSrcE,amoActiveE,scE;
...
EM_REG EM_REG (.clk(clk),.Rd(RdE),.ALUResult(ALUResultE),.WD(WriteDataE),.PC_PLUS(PCPlus4E),.RegWrite(RegWriteE),.MemWrite(MemWriteE),.ResultSrc(ResultSrcE),
                .ALUResultM(ALUResultM),.WD_M(WriteDataM),.PC_PLUS_M(PCPlus4M),.RdM(RdM),.RegWriteM(RegWriteM),
                .MemWriteM(MemWriteM), .ResultSrcM(ResultSrcM),.scE(scE),.scM(scM));
