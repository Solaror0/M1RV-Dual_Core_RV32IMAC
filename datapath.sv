...
//F Section
logic ZeroE, PCSrcE;
logic [31:0] PCPlus4D, PCPlus4F, instrD, PCD;
assign PCSrcE = (ZeroE & BranchE) | JumpE;
pc PC(.rst(rst),.clk(clk),.PCSrcE(PCSrcE),.PC_OUT(PC_OUT),.branch_addr(PCTargetE).PC_NEXT(PCPlus4F));
FD_REG FD_REG (.clk(clk),.en(),.clr(),.instrF(instrF),.PCF(PC_OUT),PCPlus4F(PCPlus4F),.PCD(PCD),.PCPlus4D(PCPlus4D),.instrD(instrD))
//ADD EN AND CLR SECTIONS AKA HAZARDS

//D Section
logic [4:0] rs1D, rs2D, RdD;
logic [31:0] RD1D, RD2D, ImmExtD;
logic [24:0] Imm_In; //check width
...
//add Extend Unit, ImmIn ImmSrcD and ImmExtD
...
DE_REG DE_REG(.clk(clk),.RD1(RD1D),.RD2(RD2D),.PC_IN(PCD),.PC_PLUS(PCPlus4D),.ImmExt(ImmExtD),.RegWrite(RegWriteD),
                ...
                .ImmSrcE(ImmSrcE),.RdE(RdE));
//ADD CLR SIGNAL in both module and here
...
//MEMORY SECTION
//DMEM connected here using ALUResumtM and WriteDataM
