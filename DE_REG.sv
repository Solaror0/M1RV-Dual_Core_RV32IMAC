...
always_ff (@posedge clk) begin
    RD1E <= RD1; RD2E <= RD2; PC_E <= PC_IN; PC_PLUS_E <=PC_PLUS; ImmExtE <= ImmExt; RegWriteE <= RegWrite; MemWriteE <= MemWrite; JumpE <= Jump;
    BranchE <= Branch; ALUSrcE <= ALUSrc; ALUControlE <= ALUControl; ALUSrcE <= ALUSrc; ALUControlE <= ALUControl;  ResultSRC_E <= ResultSRC; 
    ImmSrcE <= ImmSrc; RdE <= RdD;
