...
generate
    for (i=0; i<64; i = i+1) begin: layer1
        assign {c0[i],s0[i]} = layer0[0][i] + layer0[1][i] + layer0[2][i];
        assign {c1[i],s1[i]} = layer0[3][i] + layer0[4][i] + layer0[5][i];
        assign {c2[i],s2[i]} = layer0[6][i] + layer0[7][i] + layer0[8][i];
        assign {c3[i],s3[i]} = layer0[9][i] + layer0[10][i] + layer0[11][i];
        assign {c4[i],s4[i]} = layer0[12][i] + layer0[13][i] + layer0[14][i];
    end
endgenerate
...
genvar j;
generate
    for (j = 0; j<64; j= j+1) begin: layer2
    assign {c5[j],s5[j]}  =  layer0[15][j] + s1[j] + s2[j];
    assign {c6[j],s6[j]}  = s3[j] + s4[j] + cs0[j];
    assign {c7[j],s7[j]}  = cs1[j] + cs2[j] + cs3[j];
    end
endgenerate
...
genvar k;
generate
    for (k = 0; k<64; k= k+1) begin: layer3
    assign {c8[k],s8[k]}  = s5[k] + s6[k] + s7[k];
    assign {c9[k],s9[k]}  = cs5[k] + cs6[k] + cs7[k];
    end
endgenerate
...
genvar h;
generate
    for (h = 0; h<64; h= h+1) begin: layer4
    assign {c10[h],s10[h]}  = s8[h] + s9[h] + cs8[h];
    assign {c11[h],s11[h]}  = cs9[h] + cs4[h] + s0[h];
    end
endgenerate
...
genvar g;
generate
    for (g = 0; g<64; g= g+1) begin: layer5
    assign {c12[g],s12[g]}  =  s10[g] + s11[g] + cs10[g];
    end
endgenerate
...
genvar f;
generate
    for (f = 0; f<64; f= f+1) begin: layer6
    assign {c13[f],s13[f]}  =  cs11[f] + cs12[f] + s12[f];
    end
endgenerate
